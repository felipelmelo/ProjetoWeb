...
<body>
	...
	<!-- topbar ends -->
		...
		<div class="row-fluid">
			...
			<?php
			include_once 'repositorioOrcamento.php';
			
			$retornoObjOrcamento = RepositorioOrcamento::getInstancia()->listar();
			?>
					...
					<div class="box-content">
						<table class="table table-striped table-bordered bootstrap-datatable datatable">
						  ...
						  <?php 
						    ...
						    foreach ($retornoObjOrcamento as $ObjOrcamento){
							
						  ?>
							<tr>
								<td class="center"><?php echo utf8_encode($ObjOrcamento->getQtd()); ?></td>
								<td class="center"><?php 
									
									mysql_connect("localhost","root","");
									mysql_select_db("orcamento");

									$sql = "SELECT * FROM produto WHERE id_produto = " .$ObjOrcamento->getIdProduto();
									$qr = mysql_query($sql) or die(mysql_erro());
									if($ln = mysql_fetch_array($qr, MYSQLI_ASSOC)){
										echo $ln['nome_produto'];
									}else{
